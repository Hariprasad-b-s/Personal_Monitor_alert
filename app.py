...
def update_task(task_id):
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute('UPDATE tasks SET time_minutes = ? WHERE id = ?', (data.get('time_minutes'), task_id))
    conn.commit()
    conn.close()
...
def toggle_progress():
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute('SELECT completed FROM daily_progress WHERE task_id = ? AND date = ?', (task_id, today))
    row = cursor.fetchone()
    
    if row:
        new_status = not row['completed']
        cursor.execute('UPDATE daily_progress SET completed = ? WHERE task_id = ? AND date = ?', (new_status, task_id, today))
    else:
        cursor.execute('INSERT INTO daily_progress (task_id, date, completed) VALUES (?, ?, 1)', (task_id, today))
...
def save_timer_session():
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute('INSERT INTO timer_sessions (task_id, date, duration) VALUES (?, ?, ?)', (task_id, today, duration))
